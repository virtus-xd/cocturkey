...
(async () => {
  try {
    ...
    if (!coll) {
      figma.closePlugin(JSON.stringify({ error: `Collection "${targetName}" not found` }));
      return;
    }
    ...
    figma.closePlugin(
      JSON.stringify({ collection: coll.name, variableCount: result.length, variables: result }),
    );
  } catch (e) {
    figma.closePluginWithFailure(e.toString());
  }
...
(async () => {
  try {
    ...
    if (!node) {
      figma.closePlugin(JSON.stringify({ error: "Node not found" }));
      return;
    }
