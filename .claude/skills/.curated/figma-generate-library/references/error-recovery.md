...
(async () => {
  try {
    ...
    figma.closePlugin(
      JSON.stringify({ removed: removed.length, skipped: skipped.length, details: removed }),
    );
  } catch (e) {
...
(async () => {
  try {
    ...
    const existing = allCollections.find((c) => c.getPluginData("dsb_key") === KEY);
...
(async () => {
  try {
    ...
    const existing = page.findAll(
      (n) => n.type === "COMPONENT_SET" && n.getPluginData("dsb_key") === KEY,
    );
