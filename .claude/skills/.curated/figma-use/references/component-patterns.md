...
```javascript
for (const child of cs.children) {
  const props = Object.fromEntries(child.name.split(", ").map((p) => p.split("=")));
...
const variant =
  set.children.find((c) => c.type === "COMPONENT" && c.name.includes("size=md")) ||
  set.defaultVariant;
...
const variant =
  compSet.children.find((c) => {
    const props = Object.fromEntries(c.name.split(", ").map((p) => p.split("=")));
...
function getRelevantComponentNode(mainComponent) {
  return mainComponent.parent.type === "COMPONENT_SET" ? mainComponent.parent : mainComponent;
}
