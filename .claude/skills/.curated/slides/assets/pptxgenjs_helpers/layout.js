        ...
        const lineRectFalsePositive = (() => {
          ...
          const segsIntersect = (p1, p2, q1, q2) => {
            ...
            if (Math.abs(denom) < EPS) {
              ...
              const proj = (a, b, c) => Math.min(Math.max(a, b), Math.max(Math.min(a, b), c));
              const overlapX = !(
          ...
          if (severeTextOverlap) {
            ...
            console.error(
              `❌ ${slideLabel}: Severe text overlap detected between ${formatElement(
                a,
              )} and ${formatElement(b)} (overlap_horizontal=${comparison.intersection.w.toFixed(
                3,
...
function warnIfSlideElementsOutOfBounds(slide, pptx) {
  ...
  slide._slideObjects.forEach((obj, index) => {
    ...
    if (bounds.x2 > slideWidth + EPS)
      violations.push(`right=${bounds.x2.toFixed(3)} > width=${slideWidth.toFixed(3)}`);
    if (bounds.y2 > slideHeight + EPS)
      violations.push(`bottom=${bounds.y2.toFixed(3)} > height=${slideHeight.toFixed(3)}`);
