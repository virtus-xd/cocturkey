...
function addImageTextCard(slide, opts = {}) {
  ...
  if (!fontFace) {
    throw new Error("addImageTextCard(): textBox.fontFace is required for text measurement.");
  }
  let hText;
  let textOptions;
...
function addCardRow(slide, region, cards = [], options = {}) {
  ...
  if (options.widthStrategy === "fixed") {
    cardW = toNumberOr(options.cardWidth, rw / count - (gap * (count - 1)) / count);
  } else {
  ...
  const ox = align === "center" ? (rw - totalWidth) / 2 : align === "right" ? rw - totalWidth : 0;

  const placements = [];
  for (let i = 0; i < count; i++) {
    const x = rx + ox + i * (cardW + gap);
    placements.push(addImageTextCard(slide, { ...cards[i], x, y: ry, width: cardW }));
  }
  return placements;
}
...
function addThreeLevelTree(slide, opts = {}) {
  ...
  const rootFontFace =
    typeof rootFontFaceRaw === "string" && rootFontFaceRaw.trim().length > 0
      ...
      : null;
  if (!rootFontFace) {
    throw new Error("addThreeLevelTree(): opts.root.fontFace is required for text measurement.");
  }
  ...
  const midFontFace =
    typeof midFontFaceRaw === "string" && midFontFaceRaw.trim().length > 0
      ...
      : null;
  if (!midFontFace) {
    throw new Error("addThreeLevelTree(): opts.mid.fontFace is required for text measurement.");
  }
  ...
  let gap = Math.max(minGap, (availableRowWidth - midW * countMid) / Math.max(1, countMid - 1));
  ...
  const leafFontFace =
    typeof leafFontFaceRaw === "string" && leafFontFaceRaw.trim().length > 0
      ...
      : null;
  if (!leafFontFace) {
    throw new Error("addThreeLevelTree(): opts.leaf.fontFace is required for text measurement.");
  }
  ...
  for (let i = 0; i < midLabels.length; i++) {
    ...
    const leafGap = Math.max(minLeafGap, (midW - childCount * leafW) / Math.max(1, childCount - 1));
    ...
    for (let j = 0; j < childCount; j++) {
      ...
      addConnector(slide, xBase + midW / 2, midY + midH, x + leafW / 2, leafY, opts.line);
    }
  }
}
